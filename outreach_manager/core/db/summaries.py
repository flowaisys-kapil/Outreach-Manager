...
from outreach_manager.core.vendor.mem0.configs.prompts import get_update_memory_messages
from outreach_manager.core.vendor.mem0.memory.utils import extract_json, remove_code_blocks
...
about the lead (the specific person identified in the lead identity block below).
...
- IMPORTANT: Only extract a fact as a Lead fact when the conversation
  explicitly attributes it to the identified lead. Facts about friends,
  colleagues, clients, other companies, other named people, or examples
  given by the lead are NOT lead facts — do not record them.
...
def _build_lead_identity_context(deal) -> str:
    """Return a compact authoritative identity block for the current lead.

    Prepended to both extraction and reconciliation prompts so the model has
    an explicit anchor for who 'the Lead' is.  Prevents third-party facts
    (about the lead's friends, clients, colleagues, or other companies
    mentioned in conversation) from being recorded as lead identity facts.

    Only uses fields that are actually present on the Lead; never fabricates.
    """
    lead = deal.lead
    parts = ["Lead identity (authoritative — use this to decide what counts as a lead fact):"]
    name_parts = []
    if getattr(lead, "first_name", None):
        name_parts.append(lead.first_name.strip())
    if getattr(lead, "last_name", None):
        name_parts.append(lead.last_name.strip())
    if name_parts:
        parts.append(f"- Name: {' '.join(name_parts)}")
    if getattr(lead, "public_identifier", None):
        parts.append(f"- LinkedIn ID: {lead.public_identifier}")
    # Headline / current role from profile if available
    profile = getattr(lead, "profile", None) or {}
    headline = profile.get("headline") or getattr(lead, "headline", None)
    if headline:
        ...
        if positions:
            ...
            if role or company:
                parts.append(f"- Current role: {role} at {company}".strip(" at"))
    parts.append(
        "Only record a fact about this lead when the conversation explicitly "
        "attributes it to the person identified above. Facts about other people, "
        "companies, clients, or examples mentioned in conversation are NOT lead facts."
    )
    return "\n".join(parts)
...
def extract_facts(
    ...
    *,
    ...
    mentions in [Lead] messages to the lead.  ``lead_identity`` is a compact
    authoritative identity block for the current lead (from
    ``_build_lead_identity_context``); it prevents third-party facts from being
    recorded as lead facts.  ``context`` is an optional campaign preamble.
    ...
    if lead_identity:
        system = f"{lead_identity}\n\n{system}"
    if context:
        system = f"{system}\n\nContext for relevance:\n{context}"

    model = get_llm_model(structured_output=True)
    agent = Agent(
        model,
    ...
    result: FactList = run_agent_sync(lambda: agent.run(text), structured_output=True).output
...
def update_chat_summary(deal, new_messages, *, seller_name: str, lead_identity: str = "") -> None:
    ...
    reconciliation.  ``lead_identity`` is the compact authoritative identity
    block for this lead (pass ``_build_lead_identity_context(deal)`` from
    callers that have a Deal); it prevents third-party facts from being
    recorded as lead facts.  Empty input is a no-op.
    """
    ...
    new_facts = extract_facts(formatted, seller_name=seller_name, lead_identity=lead_identity)
    if not new_facts:
        return
...
def rebuild_chat_summary(deal, *, seller_name: str, lead_identity: str = "") -> None:
    """Rebuild ``deal.chat_summary`` from scratch using only this Deal's ChatMessages.

    Loads all ``ChatMessage`` rows for this deal, ordered chronologically,
    then runs a full extraction + reconciliation cycle.  For zero messages,
    sets ``chat_summary = {"facts": []}`` without calling the LLM.

    This function is NOT called automatically anywhere — it exists for explicit
    repair via the ``rebuild_chat_summaries`` management command or direct call.
    Running it on a deal with a contaminated summary will produce a clean one.
    """
    from outreach_manager.chat.models import ChatMessage

    messages = list(
        ChatMessage.objects.filter(deal=deal).order_by("creation_date", "pk")
    )

    if not messages:
        deal.chat_summary = {"facts": []}
        deal.save(update_fields=["chat_summary"])
        logger.info("rebuild_chat_summary: no messages for deal=%s — cleared", deal.pk)
        return

    formatted = _format_messages_for_extraction(messages)
    if not formatted:
        # Only outgoing messages — nothing to extract.
        deal.chat_summary = {"facts": []}
        deal.save(update_fields=["chat_summary"])
        logger.info("rebuild_chat_summary: only outgoing messages for deal=%s — cleared", deal.pk)
        return

    new_facts = extract_facts(formatted, seller_name=seller_name, lead_identity=lead_identity)
    # Reconcile from an empty baseline (full rebuild, not incremental).
    reconciled = reconcile_facts([], new_facts, seller_name=seller_name,
                                 lead_identity=lead_identity)
    deal.chat_summary = {"facts": reconciled}
    deal.save(update_fields=["chat_summary"])
    logger.info(
        "rebuild_chat_summary: deal=%s rebuilt with %d facts from %d messages",
        deal.pk, len(reconciled), len(messages),
    )
...
def reconcile_facts(
    ...
    *, seller_name: str, lead_identity: str = "",
) -> list[str]:
    """Reconcile ``new_facts`` against ``existing`` via mem0's UPDATE prompt.

    ``seller_name`` allows the prompt to DELETE stored facts that mistakenly
    attribute the seller's name to the lead.  ``lead_identity`` is the compact
    authoritative identity block — passed into the reconciliation prompt so the
    model can reject facts that conflict with the known lead identity or that
    describe third parties rather than the lead.
    Returns the new flat fact list after applying ADD/UPDATE/DELETE/NONE.
    """
    if not new_facts:
        return list(existing)
    actions = _request_memory_actions(existing, new_facts, seller_name, lead_identity)
    return _apply_memory_actions(existing, actions)


def _request_memory_actions(
    existing: list[str], new_facts: list[str], seller_name: str, lead_identity: str = "",
) -> list[_MemoryAction]:
    ...
    vendored ``remove_code_blocks`` / ``extract_json`` fallback chain so
    providers that wrap JSON in markdown or emit ``<think>`` blocks still
    parse cleanly.  ``lead_identity`` is prepended to anchor the model to the
    correct person and reject third-party contamination.
    """
    from pydantic_ai import Agent
    ...
    preamble_parts = []
    if lead_identity:
        preamble_parts.append(lead_identity)
    preamble_parts.append(
        ...
        f"Also DELETE any fact that contradicts the authoritative lead identity above "
        f"or that describes a third party rather than the lead."
    )
    prompt = "\n\n".join(preamble_parts) + "\n\n" + base

    # Reconciliation uses unstructured text output — backup is always eligible.
    agent = Agent(get_llm_model(structured_output=False),
                  model_settings={"temperature": 0.0, "timeout": 60})
    text = run_agent_sync(lambda: agent.run(prompt)).output

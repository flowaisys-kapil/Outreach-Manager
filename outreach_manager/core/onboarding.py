...
_ACCOUNT_KEYS = {
    "linkedin_email", "linkedin_password", "connect_weekly_limit",
...
def collect_from_wizard() -> OnboardConfig:
    ...
    """
    from outreach_manager.core.onboarding_prompts import SELF_HOSTED_QUESTIONS
    from outreach_manager.core.onboarding_wizard import ask
...
def _create_seed_leads(campaign, seed_urls: str) -> None:
    ...
    if not seed_urls or not seed_urls.strip():
        return
    from outreach_manager.linkedin.setup.seeds import parse_seed_urls, create_seed_leads
...
def apply(config: OnboardConfig) -> None:
    ...
    from outreach_manager.core.management.setup_crm import DEFAULT_CAMPAIGN_NAME
    from outreach_manager.core.models import Campaign
    from outreach_manager.linkedin.models import LinkedInProfile
    ...
    if config.legal_acceptance:
        from outreach_manager.linkedin.models import LinkedInProfile as LP

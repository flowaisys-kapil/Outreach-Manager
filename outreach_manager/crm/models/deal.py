...
class Deal(models.Model):
    ...
    # Phase 5: Deterministic scheduling & execution claiming
    ...
    # Durable timestamp: set immediately after a connection request is successfully
    # sent (PENDING transition). Used by the withdrawal check to compute true request
    # age. Must not use creation_date as a proxy — the deal may be created days before
    # the request is actually sent (e.g., on re-invite after withdrawal cooldown).
    connection_requested_at = models.DateTimeField(null=True, blank=True, db_index=True)
    ...
    # Durable guard: set immediately after send_raw_message() confirms success.
    # Once non-NULL, the first-message workflow MUST NOT send again — even if
    # post-send conversation synchronization fails and no ChatMessage exists yet.
    # Use ChatMessage history as a legacy fallback for deals created before this
    # field was added (NULL + existing ChatMessage → also protected).
    first_message_sent_at = models.DateTimeField(
        null=True,
        blank=True,
        db_index=True,
    )

...
from outreach_manager.core.conf import CAMPAIGN_CONFIG, PROMPTS_DIR
...
FORBIDDEN_PLACEHOLDERS = [
    "[lead", "[name]", "[company]", "{name}", "{first_name}", "[first_name]",
    "continued", "[insert", "[prospect", "[position]", "[title]", "hi [", "hello ["
]


def validate_and_sanitize_message(message: str, first_name: str = "") -> tuple[bool, str]:
    """Validate that a pre-generated message contains no template placeholders or AI slop.

    Returns (is_valid, clean_message).
    """
    if not message:
        return False, ""
    msg_lower = message.lower().strip()

    # Reject if any raw template placeholder string remains
    for tag in FORBIDDEN_PLACEHOLDERS:
        if tag in msg_lower:
            logger.warning("Sanitizer rejected message with placeholder '%s': %s", tag, message)
            return False, ""

    # Length check
    if len(message) < 15 or len(message) > 350:
        logger.warning("Sanitizer rejected message with length %d", len(message))
        return False, ""
...
class QualificationDecision(BaseModel):
    """Structured LLM output for joint lead qualification and pre-generated 1st message."""
    qualified: bool = Field(description="True if the profile is a good prospect, False otherwise")
    reason: str = Field(description="Brief explanation for the decision")
    first_message: str = Field(
        default="",
        description="If qualified, write a short, highly personalized 1-2 sentence first message for this prospect. Do NOT use template brackets or placeholders like [name] or [company]."
    )


def qualify_with_llm(profile_text: str, product_docs: str, campaign_objective: str, first_name: str = "") -> tuple[int, str, str]:
    """Call LLM to qualify a profile AND generate its personalized 1st message in a single pass.

    Returns (label, reason, first_message).
    ...
    decision = run_agent_sync(lambda: agent.run(prompt)).output

    label = 1 if decision.qualified else 0
    clean_msg = ""
    if label == 1 and decision.first_message:
        is_valid, clean_msg = validate_and_sanitize_message(decision.first_message, first_name=first_name)
        if not is_valid:
            clean_msg = ""

    return (label, decision.reason, clean_msg)

...
from outreach_manager.linkedin.pipeline.qualify import fetch_qualification_candidates, run_qualification
from outreach_manager.linkedin.pipeline.ready_pool import find_ready_candidate, promote_to_ready
from outreach_manager.linkedin.pipeline.search import run_search
...
def ready_source(session, qualifier: BayesianQualifier, threshold: float | None = None, backfill: bool = True) -> Generator[dict, None, None]:
    """Yield ready-to-connect candidates, optionally pulling from qualify when needed."""
    if threshold is None:
        threshold = CAMPAIGN_CONFIG["min_ready_to_connect_prob"]
    qualify = qualify_source(session, qualifier) if backfill else None
    ...
    while True:
        ...
        if qualify is not None and next(qualify, None) is not None:
            # Re-check promote after new label
            promote_to_ready(session, qualifier, threshold)
            continue

        # Upstream exhausted or backfill disabled
        return


def find_candidate(session, qualifier: BayesianQualifier, backfill: bool = True) -> dict | None:
    ...
    """
    return next(ready_source(session, qualifier, backfill=backfill), None)

# outreach_manager/linkedin/tasks/first_message.py
...
def handle_first_message(task, session, qualifiers) -> bool:
    ...
    while session.linkedin_profile.can_execute(ActionLog.ActionType.FOLLOW_UP):
        target_deal = claim_due_deal(campaign, [DealState.CONNECTED], "FIRST_MESSAGE")
        if not target_deal:
            break
        ...
        logger.info("[%s] %s %s", campaign, colored("▶ send_first_message", "cyan", attrs=["bold"]), public_id)

        deal_retry_count = 0
        cached_message_text = None

        while deal_retry_count <= 1:
            try:
                verify_ui_ready(session, target_deal)

                if cached_message_text is None:
                    ...
                    if not message_text:
                        materialize_profile_summary_if_missing(target_deal, session)
                        raw_msg = generate_first_message(session, target_deal)
                        is_valid, clean_msg = validate_and_sanitize_message(raw_msg)
                        if is_valid:
                            message_text = clean_msg

                    cached_message_text = message_text
                else:
                    message_text = cached_message_text
                ...
                if sent:
                    ...
                    logger.info("[%s] first_message sent successfully to %s", campaign, public_id)
    ...
    logger.info(
        "[%s] First Message Workflow — Processed: %d, Skipped: %d, Errors: %d",

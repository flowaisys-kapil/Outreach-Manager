...
class TestGetEmbedding:
    def test_returns_cached(self, fake_session, db):
        ...
        with patch("outreach_manager.linkedin.ml.embeddings.embed_text") as mock:
    ...
    def test_enriches_and_embeds(self, fake_session, db):
        ...
        with patch("linkedin_cli.api.client.PlaywrightLinkedinAPI") as MockAPI, \
             patch("outreach_manager.linkedin.ml.embeddings.embed_text", return_value=fake_emb):
            MockAPI.return_value.get_profile.return_value = (FAKE_PROFILE, {})
            result = lead.get_embedding(fake_session)
...
class TestResolveApiEmail:
    ...
    def test_hit_persists_email(self, db):
        from outreach_manager.crm.models import Lead
        from outreach_manager.emails.bettercontact import BetterContactQuery, BetterContactResult

        lead = self._lead()
        with patch(
            "outreach_manager.emails.bettercontact.resolve_email",
    ...
    def test_genuine_miss_returns_false(self, db):
        ...
        lead = self._lead()
        with patch("outreach_manager.emails.bettercontact.resolve_email", return_value=None):
    ...
    def test_bettercontact_unavailable_returns_none(self, db):
        from outreach_manager.crm.models import Lead
        from outreach_manager.emails.bettercontact import BetterContactUnavailable

        lead = self._lead()
        with patch("outreach_manager.emails.bettercontact.resolve_email", side_effect=BetterContactUnavailable("no key")):
    ...
    def test_already_resolved_is_noop(self, db):
        ...
        with patch("outreach_manager.emails.bettercontact.resolve_email") as resolve:
            assert lead.resolve_api_email() is True

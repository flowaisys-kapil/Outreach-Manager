...
from outreach_manager.emails import nudge
...
def _tty(yes=True):
    return patch("outreach_manager.emails.nudge.sys.stdin.isatty", return_value=yes)


def _stub_collectors(**by_state):
    return patch("outreach_manager.emails.nudge._COLLECT_BY_STATE", by_state)
...
def test_headless_logs_pending_step_without_collecting():
    ...
    with _tty(False), _stub_collectors(**{nudge.NO_BETTERCONTACT: finder, nudge.NO_MAILBOX: MagicMock()}), \
         patch("outreach_manager.emails.nudge.logger") as log:
        nudge.prompt_email_setup()
    finder.assert_not_called()
...
def test_import_stores_box_when_auth_succeeds():
    with patch("outreach_manager.emails.nudge.verify_auth", return_value=(True, "ok")):
...
def test_import_skips_box_and_records_failure_on_auth_error():
    with patch("outreach_manager.emails.nudge.verify_auth", return_value=(False, "auth rejected (534)")):
        report = nudge.import_mailboxes(_APP_PW_SHEET)
